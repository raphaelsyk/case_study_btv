...
def _quarter(quarter_name: str, framing_evidence: list[Evidence]) -> QuarterAIAnalysis:
    ...
    answer = QuestionAnswer(question='How is AI framed?', answer='framing narrative', evidence=framing_evidence)
...
def test_evidence_ids_are_deterministic_from_quarter_section_and_position() -> None:
    ...
    assert _resolved(catalogue, '2025_Q1#framing#0#0').excerpt == 'first quote'
    assert _resolved(catalogue, '2025_Q1#framing#0#1').excerpt == 'second quote'


def test_resolve_returns_none_for_unknown_id() -> None:
    catalogue = EvidenceCatalogue([_quarter('2025_Q1', [_evidence('2025_Q1', 'a quote')])])

    assert catalogue.resolve('2025_Q1#framing#0#99') is None
...
def test_resolve_returns_none_for_evidence_that_failed_grounding() -> None:
    ...
    assert catalogue.resolve('2025_Q1#framing#0#0') is None
...
def test_ids_stay_stable_across_quarters_with_the_same_section() -> None:
    ...
    assert _resolved(catalogue, '2025_Q1#framing#0#0').excerpt == 'q1 quote'
    assert _resolved(catalogue, '2025_Q2#framing#0#0').excerpt == 'q2 quote'
...
def test_render_for_prompt_includes_every_quarter_and_evidence_id() -> None:
    ...
    assert '[2025_Q1#framing#0#0]' in rendered
