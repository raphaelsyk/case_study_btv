"""Deterministic self-check that a chunk's text is actually grounded in the raw pages
...
extracted for the same pages - catching LLM paraphrasing/hallucination without a human
reviewer. A failed check flags the chunk; it never drops it or blocks storage (see the
"Grounding check" decision in system_design/02_system_design.md).
"""
...
# its claimed pages' raw text. Generous rather than strict: normalization differences
...
class GroundingChecker:
    ...
    def _is_grounded(self, chunk: Chunk, raw_text_by_page: dict[int, str]) -> bool:
        """A chunk is grounded if its text is a near-verbatim match of its pages' raw text."""
        combined_raw_text = ' '.join(raw_text_by_page.get(page_no, '') for page_no in chunk.pages)
        return self._containment_ratio(self._normalize(chunk.text), self._normalize(combined_raw_text)) >= (
            _MATCH_THRESHOLD
        )
